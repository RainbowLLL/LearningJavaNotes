...
1. JVM中的==程序计数寄存器（Program Counter Register）==中，Register的命名源于CPU的寄存器，寄存器存储指令相关的现场信息。CPU只有把数据装载到寄存器才能够运行。
...
4. 在JVM规范中，每个线程都有它自己的程序计数器，是==线程私有==的，生命周期与线程的生命周期保持一致。
...
7. 它是唯一一个在Java虚拟机规范中==没有规定任何OutofMemoryError==情况的区域。

![image-20200705155551919](https://imgconvert.csdnimg.cn/aHR0cDovL2hleWdvLm9zcy1jbi1zaGFuZ2hhaS5hbGl5dW5jcy5jb20vaW1hZ2VzL2ltYWdlLTIwMjAwNzA1MTU1NTUxOTE5LnBuZw?x-oss-process=image/format,png)

> **PC 寄存器的作用**

PC寄存器用来存储指向==下一条指令的地址==，也即将要执行的指令代码。由执行引擎将指令解释为机器码交由cpu进行处理

![image-20200705155728557](https://imgconvert.csdnimg.cn/aHR0cDovL2hleWdvLm9zcy1jbi1zaGFuZ2hhaS5hbGl5dW5jcy5jb20vaW1hZ2VzL2ltYWdlLTIwMjAwNzA1MTU1NzI4NTU3LnBuZw?x-oss-process=image/format,png)
...
![image-20200727212024900](https://imgconvert.csdnimg.cn/aHR0cDovL2hleWdvLm9zcy1jbi1zaGFuZ2hhaS5hbGl5dW5jcy5jb20vaW1hZ2VzL2ltYWdlLTIwMjAwNzI3MjEyMDI0OTAwLnBuZw?x-oss-process=image/format,png)
...
2. J==VM的字节码解释器就需要通过改变PC寄存器的值来明确下一条应该执行什么样的字节码指令==

![image-20200705161409533](https://aliyun-typora-img.oss-cn-beijing.aliyuncs.com/imgs/20201212101132.png)
...
![image-20200705161849557](https://imgconvert.csdnimg.cn/aHR0cDovL2hleWdvLm9zcy1jbi1zaGFuZ2hhaS5hbGl5dW5jcy5jb20vaW1hZ2VzL2ltYWdlLTIwMjAwNzA1MTYxODQ5NTU3LnBuZw?x-oss-process=image/format,png)